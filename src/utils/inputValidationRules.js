...
export function lengthEqualsRule(inputName, charactersNumber) {
    return createValidationRule(
        'minLength',
        `${inputName} must be of length ${charactersNumber}`,
        (inputValue, formObj) => inputValue.length === charactersNumber
    );
}
...
export function emailRule() {
    return createValidationRule(
        ...
        (inputValue, formObj) => inputValue.includes('a')
    );
}
